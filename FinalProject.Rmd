...
```{r IPW Mean Guide Tree, include = FALSE, cache = TRUE}
zclass = read.table("classfit.txt", header = TRUE)
probmissing = zclass[,6] #prob INTP is missing
group = !is.na(clean.data.2$INTP) #nonmissing INTP obs
weights = clean.data.2$PWGTP
ipw = sum(weights[group]*clean.data.2$INTP[group]/probmissing[group])/sum(weights[group]/probmissing[group])

simple.mean = sum(weights[group] * clean.data.2$INTP[group])/sum(weights[group])
```
