...
if(file.exists("logPredProbs.csv")){
  ...
  #data in log was orginally something besides mice.data I think it was no.high.level.all.obs
...
p =  1-probmissing
y = all.obs.no.high.level$INTP
gp = !is.na(y)
w = all.obs.no.high.level$PWGTP
log.est = sum(w[gp]*y[gp]/p[gp])/sum(w[gp]/p[gp])
df = data.frame(x= log.est)
kable(df, col.names = "Logistic Regression Estimate")
```

```{r randomForest Regression}
#removes high level vars, and moves INTP to first column
...
w = rf.data$PWGTP #get weights before removed from data set 
rf.data = rf.data %>% select(-c(PWGTP,FINTP)) #remove weight and flag var
predictors = rf.data[,2:dim(rf.data)[2]] #exclude first column from predictor (INTP)
rf = randomForest(y = rf.data$INTP, x = predictors, weights = w, do.trace = TRUE)
...
note that ipw method in ctree isnt working as expected (look at p in the pred file) but the estimate is somehow very reasonable.
p in logitic regression is basically all ones for some reason
