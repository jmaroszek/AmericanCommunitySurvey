...
Of the remaining variables, 43 of them have at least half of the values missing. The top ten variables that have the most missing values can be seen in the following table. Note that I have corrected for imputed values. There are many flag variables, such as FINTP, that take on values 0 or 1. In the case that FINTP = 1, I have set the corresponding INTP values to NA in order to run GUIDE analysis on the unadulterated data set. I will use my own imputation methods later in order to use methods that can not handle missing values when estimating the mean of INTP.
...
results = data.frame("GUIDE Tree Prediction" = ipw, 
                     ...
                     "Ignore Missing Values" = naive.mean)
