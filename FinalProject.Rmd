...
```{r Logisitic Regression, echo=FALSE, warning=FALSE, cache=FALSE}
...
log = glm(FINTP ~ . - INTP - PWGTP, data = all.obs.no.high.level, family = "binomial", control = list(trace = FALSE, maxit = 50))
probmissing = predict(log, newdata = mice.data, type = "response") #P(FINTP = 1) which is prob that FINTP is imputed in original and thus essentially MISSING  (so this var is not actually pi. it is 1- pi because pi refers to P(FINTP = 1))
p =  1-probmissing
