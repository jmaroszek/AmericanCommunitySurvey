...
## CTREE
I have removed categorical variables with greater than 31 levels in order to use CTREE. 
...
cf = cforest(INTP ~ . - FINTP - PWGTP, data = cforest.data, weights = w, ntree = 100, trace = TRUE)
yhat = predict(cf, newdata = cforest.data)

cf.est = (sum(w[!miss]*y[!miss]) + sum(w[miss]*yhat[miss]))/sum(w)
cf.est
```
...
Attach guide tree images to report.  
Find way to get numbered pages with all figures not looking dumb.  
